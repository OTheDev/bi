...
#include "bi_.hpp"
#include "h_.hpp"
#include "instantiate_integral.hpp"
#include "uints.hpp"
...
size_t bi_t::capacity() const noexcept { return vec_.capacity(); }

/// Return the number of digits used. Instance represents `0` iff `size() == 0`.
size_t bi_t::size() const noexcept { return vec_.size(); }

/// Reture `true` if the integer is (strictly) negative, false otherwise.
bool bi_t::negative() const noexcept { return negative_; }
 ...
 *  span.
 */
std::span<const digit> bi_t::digits() const {
  if (vec_.size() == 0) {
 ...
 *  @complexity O(1)
 */
void bi_t::swap(bi_t& other) noexcept {
  using std::swap;
 ...
 *  @complexity O(1)
 */
bool bi_t::odd() const noexcept { return size() == 0 ? false : (*this)[0] & 1; }

/**
 *  @brief Return `true` if this integer is even, `false` otherwise.
 *  @complexity O(1)
 */
bool bi_t::even() const noexcept { return !odd(); }
...
/*
 *  In order to separate the implementation of these templates from the header,
 *  we define them here and explicitly instantiate them for the integral types.
 *  TODO: Currently, this covers all integral types, but to reduce code size,
 *  it may be beneficial to only do a select few of them, as narrower types can
 *  be promoted.
 */
...
BI_INST_TEMPLATE_FOR_INTEGRAL_TYPES_ARG(bool operator<, const bi_t&);
BI_INST_TEMPLATE_FOR_INTEGRAL_TYPES_ARG(bool operator>, const bi_t&);
BI_INST_TEMPLATE_FOR_INTEGRAL_TYPES_ARG(bool operator<=, const bi_t&);
BI_INST_TEMPLATE_FOR_INTEGRAL_TYPES_ARG(bool operator>=, const bi_t&);
BI_INST_TEMPLATE_FOR_INTEGRAL_TYPES_ARG(bool operator==, const bi_t&);
